...
if ($request->query('date')) {
    ...
    public function projectStats(Request $request)
    {
        ...
        $userCounts = User::whereIn('project_id', $projectIds)
            ->selectRaw('project_id, COUNT(*) as total_staff, SUM(CASE WHEN is_absent = 0 THEN 1 ELSE 0 END) as present_staff, SUM(CASE WHEN is_absent = 1 THEN 1 ELSE 0 END) as absent_staff')
            ->groupBy('project_id')
            ->get()
            ->keyBy('project_id');
